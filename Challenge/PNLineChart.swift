...
class PNLineChart: UIView{
        ...
        didSet {
            if showLabel {
                ...
                for index in 0..<xLabels.count {
                    let labelTxt = xLabels[index] as? String
                    let labelText = String(labelTxt?.suffix(2) ?? "")
                    ...
                    label.textColor = UIColor(named: "DescribeText")
                    addSubview(label)
                }
            } else {
    ...
    public var yLabels: NSArray = []{
        didSet {
            ...
            let ySetp: CGFloat = (yValueMax - yValueMin) / CGFloat(yLabelNum * 32)
